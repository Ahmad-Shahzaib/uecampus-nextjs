...
export default function Faqs() {
  // Initialise expandedId with the first FAQ's ID when data is available
  const [expandedId, setExpandedId] = useState<string | null>(() => {
    // This callback runs only once on mount – no effect needed
    return null; // will be overridden after data loads
  });

  const dispatch = useDispatch();
  const { faqContent, data: faqs, isLoading, error } = useSelector((state) => state.faqs);

  // Fetch FAQs (unchanged)
  useEffect(() => {
  ...
  // Set first FAQ as open **once** when FAQs are loaded for the first time
  useEffect(() => {
    if (faqs.length > 0 && expandedId === null) {
      // Use functional update to avoid depending on expandedId in deps
      setExpandedId(faqs[0]._id);
    }
  }, [faqs]); // ← removed expandedId from dependencies
